...
get_TRseq <- function(gene, chain=NULL, tcrTab=NULL,
  ...
  if (!is.null(tcrTab)){
    ...
    if (gene == "V"){
      geneNames <- tcrTab[[paste0("TR", chain, gene)]]
      isDummy <- grepl("_dummy$", geneNames)
      # For _dummy gene names, strip the suffix and look up the base gene so we
      # can build a CDR1/CDR2-neutralised sequence from a real reference entry.
      geneTab <- MixTCRviz::cdr123[[species]][[paste0("TR", chain)]][
        sub("_dummy$", "", geneNames), ]
    ...
    if (any(isDummy)){
      # Replace CDR1 and CDR2 with same-length glycine runs so the dummy chain
      # has a neutral framework while CDR3 is preserved for boundary detection.
      geneSeq[isDummy] <- sub(
        geneTab$CDR2[isDummy],
        strrep("G", nchar(geneTab$CDR2[isDummy])),
        sub(geneTab$CDR1[isDummy],
          strrep("G", nchar(geneTab$CDR1[isDummy])),
          geneSeq[isDummy], fixed=TRUE),
        fixed=TRUE)
    }
        ...
        if (cCol %in% c("TCRa", "TCRb")){
          ...
          for (cSubCol in subCols){
            ...
            if (!is.null(cMSA) && !is.null(x[[paste0("n_", cSubCol)]])){
              ...
              n_add <- nAA + n_trim - nchar(cMSA$sequence)
              if (grepl("TR.V", cSubCol)){
                pattern <- paste0("([[:lower:]]*[-[:upper:]]){", n_trim, "}",
                  "(\\n(>|$))")
                repl <- paste0(stringr::str_dup("-", times=n_add), "\\2")
                # We need to remove some AA from the end of the aligned
                # sequences (i.e., those AA deleted during VDJ recombination),
                # and to add at the end of the sequences some gaps corresponding
                # to the remaining CDR3 + J sequence parts. Note that we need to
                # remove exactly n_trim '-' or upper-case letters, but there
                # can be any number of lower-case letters in this trimmed region
                # that we also need to remove as these correspond to inserted
                # AAs in the aligned sequences in comparison to the query
                # sequence.
              } else if (grepl("TR.J", cSubCol)){
                pattern <- paste0("(\\n)(?!(>|$))([-[:upper:]][[:lower:]]*){",
                  n_trim, "}")
                repl <- paste0("\\1", stringr::str_dup("-", times=n_add))
                # For TRxJ, we need to remove AAs and add gaps at the beginning
                # of the aligned sequences. We therefore search for newlines
                # not followed by '>' (that would indicate a descriptor line,
                # not a sequence of AAs).
              } else {
                stop("cSubCol doesn't seem to match TR.V/J: ", cSubCol)
              }
              cMSA$unpairedMsa <- gsub(pattern, repl, x=cMSA$unpairedMsa, perl=T)
              cMSA$pairedMsa <- gsub(pattern, repl, x=cMSA$pairedMsa, perl=T)
          ...
          if (!is.null(pStruct[[1]]$protein$unpairedMsa) ||
            ...
            # Could obtain some pre-aligned MSA. Need to add the query line
            # on top as first alignment.
            qu_str <- paste0(">query\n", tcrSeq, "\n")
