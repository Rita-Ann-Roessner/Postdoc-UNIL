...
#' @export
plotVJ <- function(count.es, count.rep, plddt.es = NULL, sd.es=NULL, sd.rep=NULL, distr.es=NULL, distr.rep=NULL, info=NULL, comp.baseline=T, pType=1,
  ...
  if (is.null(combined.resList)){
    ...
    # Add pLDDT values aligned by gene/name
    count.df$plddt <- NA_real_
    if (!is.null(plddt.es)) {
      # allow names either on full allele-stripped names or on gene names
      plddt_tmp <- plddt.es
      
      # first try exact match to count.df$name
      idx_name <- match(count.df$name, names(plddt_tmp))
      matched_name <- !is.na(idx_name)
      if (any(matched_name)) {
        count.df$plddt[matched_name] <- as.numeric(plddt_tmp[idx_name[matched_name]])
      }
      
      # then fill remaining by gene-level match
      idx_gene <- match(count.df$gene, names(plddt_tmp))
      matched_gene <- is.na(count.df$plddt) & !is.na(idx_gene)
      if (any(matched_gene)) {
        count.df$plddt[matched_gene] <- as.numeric(plddt_tmp[idx_gene[matched_gene]])
      }
    }
  ...
  if (floor(pType) == 1) {
    ...
    if (pType == 1.3) {
      ...
      if (pType == 1.2) {
        #count.df$shape_color <- "common"
        shapeScale <- c(common = 21)
        outerColorScale <- c(common = "gray20")
      } else {
        #count.df$shape_color <- count.df$gene
