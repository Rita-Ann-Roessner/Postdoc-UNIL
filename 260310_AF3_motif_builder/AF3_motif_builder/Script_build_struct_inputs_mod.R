...
if (Sys.info()["sysname"] == "Linux"){
  ...
  source(paste0("/work/FAC/FBM/LLB/dgfeller/epitope_pred/rroessne/260330_af3_benchmark/",
    "Struct_preds_fun.R"))
...
#   "AF3_models/")
preMSA_folder <- paste0("/work/FAC/FBM/LLB/dgfeller/epitope_pred/rroessne/260330_af3_benchmark/preMSA/",
  "AF3_models/")
...
if (is.null(MHC_allele_b)){
  if ("MHC_allele_b" %in% colnames(tcrTab)){
    ...
    if (all(grepl("^HLA_(A|B|C|E)", MHC_allele_a))){
      MHC_allele_b <- "none"
      #MHC_allele_b <- "B2M"
    } else {
...
badIds <- which((!grepl(paste0("^[", paste(c("-", MHC2R::aa_letters), collapse=""),
  "]*$"), paste0(tcrTab$TCRa, tcrTab$TCRb))) |
    is.na(TRAV$seq) | is.na(tcrTab$cdr3_TRA) | is.na(TRAJ$seq) |
    is.na(TRBV$seq) | is.na(tcrTab$cdr3_TRB) | is.na(TRBJ$seq))
# Check if unknown AAs are contained in the sequences or if some genes/CDR3 were
# unknown (e.g. only beta chain was obtained for given gene)
if (length(badIds) > 0){
  warning(length(badIds), " TCRs contain bad AAs in their sequence (e.g. ",
    paste(tcrTab$id[badIds[1:3]], collapse=", "), " with TCRa: ",
    paste(tcrTab$TCRa[badIds[1:3]], collapse=", "), ", TCRb: ",
    paste(tcrTab$TCRb[badIds[1:3]], collapse=", "),
    "). Removing these from the exported TCRs.")
