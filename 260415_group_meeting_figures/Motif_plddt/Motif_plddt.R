...
df <- read.table(
  file.path(topdir, "YF_LAU5013_sc_WT_output.txt"),
  header = TRUE
)[, c("id", "AF3_iptm_pair_mean")]

anno <- read.table(
  file.path(topdir, "YF_LAU5013_sc_WT_input.txt"),
  header = TRUE
)
...
motif <- MixTCRviz(input1 = model)
baseline.model <- MixTCRviz::baseline_HomoSapiens

df_plddt <- read.csv(file.path(topdir, "YF_cdr_plddts.csv"))
...
ggsave(
  file.path(topdir, "YF_motif_plddt.png"), 
  fig,
  width = 20,
  height = 10,
  dpi = 300
